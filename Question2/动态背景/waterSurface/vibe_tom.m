...
%                 随机改变(i,j)某个背景邻域像素点的某个样本值
  ...
  video_dis= imfill(video_dis,'holes');%将原图填充孔洞
    ...
    video_dis=imreconstruct(imerode(video_dis,strel('disk',4)),video_dis);    
    
     frameNum = f -18 ;     %这里的0表示截取的开始帧数
     path='D:\数学建模代码\2017\D\附件2-典型视频\不带晃动-动态背景\waterSurface\result\';
