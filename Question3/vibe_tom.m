...
%      subplot(1,2,2),imshow(video_dis,[]);title(sprintf('第%d帧目标', f), 'FontWeight', 'Bold', 'Color', 'r');
    ...
    video_dis=imreconstruct(imerode(video_dis,strel('square',2)),video_dis);   
     frameNum = f - 16;     %这里的0表示截取的开始帧数
     path='D:\数学建模代码\问题3\结果1\';
     imwrite(video_dis,strcat(path,int2str(frameNum),'.jpg'));
  


     figure(2),subplot(1,2,1),imshow(image,[]);title(sprintf('第%d帧视频', f), 'FontWeight', 'Bold', 'Color', 'r');
    subplot(1,2,2),imshow(out,[]);title(sprintf('第%d帧背景', f), 'FontWeight', 'Bold', 'Color', 'r');
     subplot(1,2,2),imshow(video_dis,[]);title(sprintf('第%d帧目标', f), 'FontWeight', 'Bold', 'Color', 'r');
     %%drawnow;
end
